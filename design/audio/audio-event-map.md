...
| 事件 | 定义位置 | 现状 | 影响 / 决策点 |
|---|---|---|---|
| `ON_JUMP` | event-bus.ts:7 | **仅** `core/sim/headless.ts:180` emit（仿真），真实 game-scene 不发 | 跳跃无声（D1） |
| `ON_DOUBLE_JUMP` | event-bus.ts:8 | 从未 emit | 二段跳无音（D5） |
| `ON_ENEMY_HIT_PLAYER` | event-bus.ts:12 | 从未 emit（`ON_HURT` 覆盖玩家受击） | 与 `ON_HURT` 合并即可，不单列 |
| `ON_PROJECTILE_SPAWN` | event-bus.ts:13 | 从未 emit（`shi_pao` 仅返回 `Projectile[]`） | 石炮开火无声（D3） |
| `ON_LIFE_LOST` | event-bus.ts:24 | 从未 emit（`ON_DEATH`/`ON_HURT` 覆盖） | 可不单列 |
...
| `ON_FORM_CHANGED` | event-bus.ts:32 | 从未 emit（GDD 06 元气果 form 变化，MVP 无） | 留待道具系统 |
| `ON_LEVEL_COMPLETE_UI` | event-bus.ts:33 | 从未 emit | UI 变体，可不用 |
